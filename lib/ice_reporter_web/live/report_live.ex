defmodule IceReporterWeb.ReportLive do
  ...
  defp search_addresses(query) do
    ...
    params = [
      format: "json",
      q: query,
      limit: 8,  # Increased limit to get more results for landmarks
      countrycodes: "us",
      addressdetails: 1,
    ...
    case Req.get(url,
      ...
      {:ok, %{status: 200, body: results}} when is_list(results) ->
        suggestions =
          ...
          |> Enum.take(5)  # Take top 5 after filtering
  ...
  defp detect_landmark_or_area(result, address) do
    ...
    cond do
      # Bridges
      address["bridge"] -> "#{address["bridge"]}, #{address["city"] || address["county"]}, #{address["state"]}"
      ...
      address["leisure"] -> "#{address["leisure"]}, #{address["city"] || address["county"]}, #{address["state"]}"
      ...
      # Water features
      address["natural"] -> "#{address["natural"]}, #{address["city"] || address["county"]}, #{address["state"]}"
      
      # Check display_name for common landmarks
      result["display_name"] && String.contains?(String.downcase(result["display_name"]), ["bridge", "highway", "freeway", "park", "airport", "hospital", "mall", "center", "plaza"]) ->
  ...
  defp format_display_name_fallback(display_name) when is_binary(display_name) do
    ...
    case parts do
      ...
      [first | rest] ->
        ...
        state_part = Enum.find(rest, fn part -> 
  ...
  defp verify_hcaptcha_token(token) do
    secret_key = Application.get_env(:ice_reporter, :hcaptcha_secret, "0x0000000000000000000000000000000000000000")
